...
def detect_auth_header_failures(msg):
    ...
    if not msg:
        return {"detected": False, "severity": "NONE", "reason": "No email metadata", "indicators": [],
                "status": auth_status}

    indicators = []

    # Collect all authentication-related headers
...
def detect_url_mismatch(soup):
    """Detects when the visible link text differs from the actual href destination."""
    if not soup:
        return {"detected": False, "severity": "NONE", "indicators": []}
